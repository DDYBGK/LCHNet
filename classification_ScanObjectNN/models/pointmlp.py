...
class PointAgent(nn.Module):
    ...
    def forward(self, qakv):
        ...
        return output.transpose(1, 2), pre_features  # 恢复通道维度 [B,1024,512]
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.after_x_pool = nn.AdaptiveMaxPool1d(128)  # 从512点下采样到128
        self.third_stage = nn.AdaptiveMaxPool1d(64)
    ...
    def forward(self, x):
        ...
        x_org = x
        after_x, prefeatures = self.point_agent(pos_features)  # B 1024 512
        pooled_after_x = self.after_x_pool(after_x)
        pooled_third = self.third_stage(prefeatures[2])
        #pooled_x_org = F.adaptive_max_pool1d(x_org, output_size=32)
        x_cat = torch.cat((x_org, pooled_third), dim=-1)
        x_final = pooled_after_x + x_cat
        #x_final = torch.cat((x_cat, pooled_after_x), dim=-1)  #128 + 128
