...
class PointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        self.class_num = num_classes        #50个类别
        self.points = points
    ...
    def forward(self, x, norm_plt, cls_label):
        xyz = x.permute(0, 2, 1)  #2 3 2048->2 2048 3
        ...
        pool = torch.nn.AdaptiveAvgPool1d(8)
        q = qakv[0]             #2 1024 512
        a = qakv[1]             #2 1024 128
        k = qakv[2]             #2 1024 32
        v = qakv[3]             #2 1024 8
        ...
        mha_feat_global = mha_feat_global.repeat(1, 1, 2048)  #重复2048次将重复后的加在整体的2048个点上  2, 1024 2048 批个数 特征 点数
        ...
        for i in range(len(self.decode_list)):
            x = self.decode_list[i](xyz_list[i+1], xyz_list[i], x_list[i+1],x)  #最终输出x 2, 128, 2048  批个数 特征 点数
        x = torch.cat((mha_feat_global,x), dim = 1)  #将QAKV获得的特征与上采样之后的每个点concat
