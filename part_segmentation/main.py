...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    model.train()
    '''
        poitns: b,n,3   表示每个点的xyz坐标
        label: b,1      表示每个物体的类别标签，例如 椅子、桌子
        target: b,n     表示每个物体中每个点的类别标签，例如 飞机机翼、桌腿、椅背等等
        norm_plt: b,n,3 表示每个点的法向量等特征
    '''
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        batch_size, num_point, _ = points.size()    # b,n,3
        #在 PyTorch 早期版本中，Variable 是用于封装张量并支持自动求导的类。
        ...
        norm_plt = norm_plt.transpose(2, 1) #b,3,n
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        ...
        # instance iou without considering the class average at each batch_size:
        '''
            batch_shapeious:存放了当前batch中每个样本含有的类的iou
            比如，一个batch含有2个样本，这两个样本分别是椅子和桌子，那么batch_shapeious=[iou_椅子,iou_桌子]
            batch_shapeious[shape_idx]表示当前batch中第shape_idx个样本的类平均iou（类平均iou: 一个样本中
            含有n个类，每个类的iou相加后除以n）
        '''
        ...
        for shape_idx in range(seg_pred.size(0)):  # sample_idx
            cur_gt_label = label[shape_idx]  # 该行代码为了获取到当前样本的类别，比如 台灯->0 电视->1 桌子->2
            final_total_per_cat_iou[cur_gt_label] += batch_shapeious[shape_idx]  #将当前样本的类平均iou加到对应类别的iou之和中
            final_total_per_cat_seen[cur_gt_label] += 1  # 统计当前某个类别样本出现的数量
