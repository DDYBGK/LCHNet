...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            interpolated_points = points2.repeat(1, N, 1)  # b n*s d''
        else:
...
class PointMLP(nn.Module):
    def __init__(self, points=4096, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 gmp_dim=64, cls_dim=64, cls_num=13, **kwargs):
        super(PointMLP, self).__init__()
        self.stages = len(pre_blocks)  # 4个阶段
        #self.class_num = num_classes  # 13个类别
        self.points = points
        ###########
        ###########
        ###########
        ...
        # self.percent_fc = nn.Sequential(
        #     nn.Conv1d(in_channels=1024, out_channels=512, kernel_size=1, bias=False),
        #     nn.BatchNorm1d(512),
        #     nn.ReLU()
        # )
        # 多头注意力机制mha
        ...
        self.act = get_activation(activation)
        #语义分割分割头
        self.conv1_cls = nn.Conv1d(256, 128, 1)
        self.bn1_cls = nn.BatchNorm1d(128)
        self.drop1_sem = nn.Dropout(0.5)
        self.conv2_cls = nn.Conv1d(128, cls_num, 1)
        #######后面其实没有用了#########
        # class label mapping
        ...
        #     nn.Conv1d(gmp_dim + cls_dim + de_dims[-1] + 128, 128, 1, bias=bias),
        #     nn.BatchNorm1d(128),
        #     nn.Dropout(),
    ...
    def forward(self, x):
        xyz = x.permute(0, 2, 1)[:,:,:3]  # 2 9 2048->2 2048 9->2 2048 3
        #x = torch.cat([x, norm_plt], dim=1)  # 2 6 2048
        x = self.embedding(x)  # B,D,N 2 64 2048

        feature_buffer = []
        xyz_list = [xyz]  # [B, N, 3]  2 2048 3
        x_list = [x]  # [B, D, N]  2 64 2048
        ...
        v = v.permute(0, 2, 1)
        a_asQ = (pool(a)).permute(0, 2, 1)  # 48 1024 8
        new_v, _ = self.mha(a_asQ, k_pooled, v)  # 48 1024 8
        q_pooled = (pool(q)).permute(0, 2, 1)  # 48 1024 512 -> 48 1024 8
        a_asK = a_asQ
        mha_feat_local, _ = self.mha(q_pooled, a_asK, new_v)  # b 1024 g
        mha_feat_local = F.adaptive_max_pool1d(mha_feat_local, 128)  # 设定比例512是4:1 384是3:1 256是2:1 128是1:1
        mha_feat_local = mha_feat_local.permute(0, 2, 1)  # 2 p 8
        mha_feat_global = F.adaptive_max_pool1d(mha_feat_local, 1)  # 2 p 1
        mha_feat_global = mha_feat_global.repeat(1, 1, 4096)  # 重复2048次将重复后的加在整体的2048个点上  2, 1024 2048 批个数 特征 点数
        # mha_feat_global = F.adaptive_max_pool1d(mha_feat_global.permute(0, 2, 1),128).permute(0, 2, 1)
        # print(f"mha_feat_global的形状是{mha_feat_global.shape}")

        # here is the decoder
        xyz_list.reverse()
        x_list.reverse()
        x = x_list[0]
        # 将之前的和经过QAKV的特征进行比重相加
        # x = 0.5*x + 0.5*mha_feat_local
        for i in range(len(self.decode_list)):
            x = self.decode_list[i](xyz_list[i + 1], xyz_list[i], x_list[i + 1], x)  # 最终输出x 2, 128, 4096  批个数 特征 点数
        x_cat = torch.cat((mha_feat_global, x), dim=1)  # 将QAKV获得的特征与上采样之后的每个点concat
...
##这里的PointMLP中的num_classes没用了改为使用cls_num
def pointMLP(num_sem=13, **kwargs) -> PointMLP:
    return PointMLP(points=4096, embed_dim=64, groups=1, res_expansion=1.0,
                    activation="relu", bias=True, use_xyz=True, normalize="anchor",
                    dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                    k_neighbors=[32, 32, 32, 32], reducers=[4, 4, 4, 4],
                    de_dims=[512, 256, 128, 128], de_blocks=[4, 4, 4, 4],
                    gmp_dim=64, cls_dim=64, cls_num=num_sem, **kwargs)


if __name__ == '__main__':
    data = torch.rand(2, 3, 2048)
    norm = torch.rand(2, 3, 2048)
    points = torch.rand(6,9,4096)
    cls_num = 13
    print("===> testing modelD ...")
    model = pointMLP(13)
    out = model(points)  # [2,2048,13]
    print(out.shape)
